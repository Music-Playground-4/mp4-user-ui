...
import { concertsApi, usersApi, displayName, type RecruitPost, type UserProfile } from '@/lib/api';
...
export default function PerfApplyPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const post = useAsync<RecruitPost>(() => concertsApi.detail(id, token), [id, token]);
  ...
  const submit = async () => {
    ...
    setError(null);
    try {
      await concertsApi.apply(token, id, {
    ...
    return (
      <>
        ...
        <div className="scroll-region">
          <ErrorState message={post.error ?? '공연을 불러오지 못했어요'} onRetry={post.reload} />
        </div>
      </>
    );
  }
  ...
  const dateLabel = p.date ? new Date(p.date).toLocaleDateString('ko-KR', { month: 'long', day: 'numeric', weekday: 'short' }) : null;
    ...
    <>
      ...
      <div className="scroll-region" style={{ padding: '16px 16px 16px' }}>
        <div style={{ padding: 12, borderRadius: 10, background: 'var(--neutral-99)', marginBottom: 20 }}>
          <div style={{ fontSize: 11, color: 'var(--fg-alternative)', marginBottom: 2 }}>지원하는 공연</div>
          ...
          <div style={{ fontSize: 11, color: 'var(--fg-alternative)', marginTop: 4 }}>
            {[p.venue ?? p.location, dateLabel].filter(Boolean).join(' · ')}
          </div>
        </div>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <textarea
            ...
            placeholder="연주 경력, 지원 동기, 가능한 셋 구성 등을 적어 주세요."
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <input
            ...
            placeholder="유튜브·사운드클라우드 공연 영상 링크"
            style={{ width: '100%' }}
          />
        </div>
