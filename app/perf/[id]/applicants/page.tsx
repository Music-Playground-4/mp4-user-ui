...
import { concertsApi, displayName, applicantOf, type Application, type Paged, type RecruitPost } from '@/lib/api';

export default function PerfApplicantsPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const { token, status } = useAuth();

  const post = useAsync<RecruitPost>(() => concertsApi.detail(id, token), [id, token]);
  const list = useAsync<Paged<Application>>(
    token ? () => concertsApi.applicants(token, id) : null,
    [token, id],
  );
  ...
  const decide = useCallback(
    async (appId: string, next: 'ACCEPTED' | 'REJECTED') => {
      ...
      setError(null);
      try {
        await concertsApi.decide(token, id, appId, next);
      } catch (e) {
    ...
    <>
      ...
      <div className="scroll-region stagger" style={{ padding: '12px 16px 24px' }}>
        ...
        {!list.loading && list.error && (
          <ErrorState
            message={list.status === 403 ? '공연 주최자만 지원자를 볼 수 있어요' : list.error}
            onRetry={list.status === 403 ? undefined : list.reload}
          />
        )}

        {!list.loading && !list.error && applicants.length === 0 && (
          <EmptyState message="아직 지원자가 없어요" hint="공연 모집글을 공유해 보세요." />
        )}
